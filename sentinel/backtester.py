...
class SimulationDatabase:
    """
    In-memory database for backtesting that implements the Database interface.
    ...
    def conn(self) -> aiosqlite.Connection:
        if not self._connection:
            raise RuntimeError("Simulation database not connected")
        return self._connection

    async def initialize_from(self, source_db: Database):
    ...
    async def _copy_table(self, source_db: Database, table: str):
    ...
    # -------------------------------------------------------------------------
    # Database interface methods - delegate to the real Database class methods
    # but operate on our in-memory connection
    # -------------------------------------------------------------------------
    ...
    async def get_cash_balances(self) -> dict[str, float]:
        ...
        return {row['currency']: row['amount'] for row in await cursor.fetchall()}
    ...
    async def set_cash_balances(self, balances: dict[str, float]) -> None:
        await self._connection.execute("DELETE FROM cash_balances")
    ...
    async def get_trades(self, symbol: str = None, limit: int = 100) -> list[dict]:
        """Get trade history from simulation database."""
