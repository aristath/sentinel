...
class OHLCValidation:
    ...
    def needs_interpolation(self) -> bool:
        ...
        return not (self.open_valid and self.high_valid and
                   self.low_valid and self.close_valid)

    def all_valid(self) -> bool:
        """Returns True when all components are valid."""
        return (self.open_valid and self.high_valid and
                self.low_valid and self.close_valid)
...
class PriceValidator:
    ...
    def validate_price(
        ...
        if close <= 0:
            return OHLCValidation(
                open_valid=False, high_valid=False,
                low_valid=False, close_valid=False,
    ...
    def _interpolate_full(
        ...
        if before and after:
            ...
            open_ratio = (before[0].get('open', before_close) / before_close +
                         after[0].get('open', after_close) / after_close) / 2.0
            high_ratio = (before[0].get('high', before_close) / before_close +
                         after[0].get('high', after_close) / after_close) / 2.0
            low_ratio = (before[0].get('low', before_close) / before_close +
                        after[0].get('low', after_close) / after_close) / 2.0
    ...
    def _get_typical_high_close_ratio(
        self,
        before: list[dict],
        after: list[dict]
    ) -> float:
    ...
    def _get_typical_low_close_ratio(
        self,
        before: list[dict],
        after: list[dict]
    ) -> float:
    ...
    def validate_and_interpolate(self, prices: list[dict]) -> list[dict]:
        ...
        if prices and interpolation_count / len(prices) > 0.5:
            logger.warning(f"More than 50% of prices flagged invalid "
                           f"({interpolation_count}/{len(prices)})")
...
def get_price_anomaly_warning(current_price: float, historical_prices: list[float], symbol: str = "") -> Optional[str]:
    ...
    if ratio >= MAX_PRICE_MULTIPLIER:
        return f"Price spike detected: {current_price:.2f} is {ratio:.1f}x the 30-day average ({avg_price:.2f}). Trades blocked."

    if ratio <= MIN_PRICE_MULTIPLIER:
        return f"Price crash detected: {current_price:.2f} is {ratio:.1f}x the 30-day average ({avg_price:.2f}). Verify before trading."
