    ...
    this.innerHTML = `
      <div class="bg-gray-800 border border-gray-700 rounded p-3" x-data>
        <div class="flex items-center justify-between mb-3">
          <h2 class="text-xs text-gray-300 uppercase tracking-wide">Recent Trades</h2>
        </div>

        <div x-show="$store.app.trades.length === 0" class="text-center py-6 text-gray-300 text-sm">
          No trades yet
        </div>

        <div x-show="$store.app.trades.length > 0" class="overflow-x-auto">
            ...
            <tbody class="divide-y divide-gray-800">
              <template x-for="trade in ($store.app.trades || [])" :key="trade.id">
                <tr class="hover:bg-gray-800/50" :class="trade.symbol.includes('/') ? 'bg-gray-800/30' : ''">
                  <td class="py-1.5 px-2 text-gray-300" x-text="formatDateTime(trade.executed_at)"></td>
                  <td class="py-1.5 px-2 font-mono"
                      :class="trade.symbol.includes('/') ? 'text-purple-400' : 'text-blue-400'"
                      x-text="trade.symbol"></td>
                  <td class="py-1.5 px-2 text-gray-300 truncate max-w-32 hidden sm:table-cell"
                  ...
                  <td class="py-1.5 px-2 text-right font-mono text-gray-300 hidden sm:table-cell" x-text="formatCurrency(trade.price)"></td>
