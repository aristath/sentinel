...
try:
    ...
    if isinstance(actions, list):
        ...
        dividends = [a for a in executed if "dividend" in str(a.get("type", "")).lower()]
        if dividends:
            print(f"\nSample dividend (first of {len(dividends)}):")
            sample = dividends[0]
            print(json.dumps({k: v for k, v in sample.items() if k in [
                "id", "type", "ticker", "currency", "ex_date", "pay_date", 
                "rate", "amount_per_one", "executed_count", "executed"
...
try:
    ...
    if isinstance(trades, dict):
        ...
        if trade_list:
            ...
            sample = trade_list[0]
            print(f"\nSample trade fields:")
            print(json.dumps({k: v for k, v in sample.items() if any(keyword in k.lower() for keyword in [
                "fee", "commission", "cost", "price", "amount", "currency", "date"
            ])}, indent=2, default=str))
            
            # Look for commission/fee fields
            fee_fields = [k for k in sample.keys() if "fee" in k.lower() or "commission" in k.lower() or "cost" in k.lower()]
            if fee_fields:
