...
async def test_get_job_schedules_returns_all(db):
    ...
    await db.conn.execute(
        """INSERT INTO job_schedules
           (job_type, enabled, interval_minutes, description, category, created_at, updated_at)
           VALUES (?, 1, 30, 'Test job 1', 'sync', ?, ?)""",
        ('sync:test1', now, now)
    )
    await db.conn.execute(
        """INSERT INTO job_schedules
           (job_type, enabled, interval_minutes, description, category, created_at, updated_at)
           VALUES (?, 1, 60, 'Test job 2', 'analytics', ?, ?)""",
...
async def test_get_job_schedule_returns_single(db):
    ...
    await db.conn.execute(
        """INSERT INTO job_schedules
           ...
           VALUES (?, 1, 30, 5, 0, 'Sync portfolio', 'sync', ?, ?)""",
...
async def test_upsert_job_schedule_inserts_new(db):
    ...
    assert schedule['dependencies'] == '["dep1"]'
...
async def test_seed_default_job_schedules_inserts_all(db):
    ...
    schedules = await db.get_job_schedules()
    assert len(schedules) >= 12  # At least 12 default schedules
    ...
    assert ml_retrain['is_parameterized'] == 1
    assert ml_retrain['parameter_source'] == 'ml_enabled_securities'
    assert ml_retrain['parameter_field'] == 'symbol'
...
@pytest.mark.asyncio
async def test_parameterized_job_schedule(db):
    """Parameterized job schedules should store parameter info correctly."""
    await db.upsert_job_schedule(
        job_type='ml:retrain',
        enabled=True,
