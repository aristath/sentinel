...
class TestDecayAiResearchMultipliers:
    """The scheduled job that fades stored ai_research_multipliers toward neutral.
    ...
    def db_with_rows(self, mock_db):
        ...
        rows = [
            {"symbol": "OLD.US", "ai_research_multiplier": 0.7, "ai_research_multiplier_updated_at": old_iso},
    ...
    async def test_old_rows_get_one_decay_step(self, db_with_rows, mock_settings):
        from sentinel.jobs.tasks import decay_ai_research_multipliers

        await decay_ai_research_multipliers(db_with_rows, mock_settings)

        # OLD.US (0.7, 10 days old) -> 0.5 + 0.2 * 0.9 = 0.68
        called_for = {c.args[0]: c.args[1] for c in db_with_rows.set_ai_research_multiplier.await_args_list}
