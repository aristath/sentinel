...
async def format_market_status_for_display(has_recommendations: bool = False) -> str:
    ...
    Returns:
        Empty string (market status messages removed for clean stats/ticker-only display)
    """
    # Return empty string - no market status messages on display
    # Display shows only: portfolio value, cash, recommendations, or system stats
    return ""
