...
func TestRegisterRoutes(t *testing.T) {
	...
	handler := NewHandlers(zerolog.Nop())
	...
	testCases := []struct {
		...
		{"POST", "/scoring/score", "ScoreSecurity"},
	}
...
func TestRegisterRoutes_RoutePrefix(t *testing.T) {
	// Verify that routes are registered under /scoring prefix
	handler := NewHandlers(zerolog.Nop())

	router := chi.NewRouter()
	handler.RegisterRoutes(router)

	// Test that routes outside /scoring prefix return 404
	req := httptest.NewRequest("POST", "/score", nil)
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
	assert.Equal(t, http.StatusNotFound, rec.Code, "Route without /scoring prefix should return 404")
}
