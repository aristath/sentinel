    ...
    this.innerHTML = `
      <div class="bg-gray-800 border border-gray-700 rounded p-3" x-data="{
        ...
        <div class="flex items-center justify-between mb-3">
          ...
          <button @click="$store.app.fetchMultiStepRecommendations(); $store.app.fetchAllStrategyRecommendations()"
                  class="p-1 text-gray-400 hover:text-gray-200 rounded hover:bg-gray-700 transition-colors"
                  :disabled="$store.app.loading.multiStepRecommendations || $store.app.loading.allStrategyRecommendations"
                  title="Refresh optimizer recommendations">
            <span x-show="$store.app.loading.multiStepRecommendations || $store.app.loading.allStrategyRecommendations" class="inline-block animate-spin">&#9696;</span>
            <span x-show="!$store.app.loading.multiStepRecommendations && !$store.app.loading.allStrategyRecommendations">&#8635;</span>
          </button>
        </div>
        ...
        <!-- Empty state - only shown when optimizer has no recommendations -->
        <template x-if="!$store.app.loading.multiStepRecommendations && !$store.app.loading.allStrategyRecommendations && !hasStrategicRecs() && !$store.app.multiStepRecommendations?.steps?.length">
          <div class="text-gray-500 text-sm py-4 text-center">
            <div>No optimized trades pending</div>
        ...
        <!-- NO FALLBACK TO HEURISTIC RECOMMENDATIONS -->
        <!-- The UI only shows optimizer-based recommendations (multi-step/strategic) -->
        <!-- This ensures what you see = what will actually be traded -->
